window.addEventListener('load', () => {
  let groups = [
    ...
    },
    {
      ...
      questions: [
        ...
        'ATM',
        '紫评表',
        '阅读<a href="/html/books/虐恋亚文化/">《虐恋亚文化》</a>',
      ],
    },
  ];
  ...
  if (searchParams.get('uid') || (Object.keys(formdata).length <= 0 && MoeApp.user.id > 0)) {
  ...
  MoeApp.login().then(() => {
    ...
    if (!searchParams.get('uid') && MoeApp.user.id > 0) {
  ...
  document.addEventListener('click', (e) => {
    if (e.target.closest('.btn.link')) {
      ...
      if (e.target.closest('.btn.link1')) {
        url = `https://t.me/moeGameBot?startapp=tests_bdsm_${uid}`;
